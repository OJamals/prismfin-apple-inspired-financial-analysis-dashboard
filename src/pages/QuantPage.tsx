...
const MOCK_ACTIVITY = [
  { month: 'Jul', volume: 4200 },
  { month: 'Aug', volume: 3800 },
  { month: 'Sep', volume: 5100 },
  { month: 'Oct', volume: 4600 },
...
export function QuantPage() {
  ...
  const refreshMutation = useMutation({
    ...
    onSuccess: () => {
      // Standardize on invalidation to ensure all related caches are refreshed
      queryClient.invalidateQueries({ queryKey: ['quant', range, mode] });
          ...
          <AnimatePresence mode="popLayout">
            {isLoading ? (
              ...
              >
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                      <Card className="lg:col-span-7 rounded-4xl border-none shadow-soft bg-card overflow-hidden">
                        <CardHeader className="p-8 pb-0">
                          <CardTitle className="text-xl font-bold font-display">Monthly Volume Audit</CardTitle>
                        </CardHeader>
                        <CardContent className="h-[340px] p-8 pt-4">
                          <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={MOCK_ACTIVITY}>
                              ...
                              <YAxis hide />
                              <Tooltip
                                contentStyle={{ borderRadius: '20px', border: 'none', boxShadow: '0 20px 40px -5px rgba(0,0,0,0.1)', padding: '16px' }}
                                cursor={{ fill: '#f8fafc', radius: 12 }}
                              />
                              <Bar dataKey="volume" radius={[12, 12, 12, 12]} barSize={40}>
                                {MOCK_ACTIVITY.map((entry, index) => (
                                  <Cell key={`cell-${index}`} fill={index % 2 === 0 ? '#14B8A6' : '#0EA5E9'} fillOpacity={0.8} />
                                ))}
                              </Bar>
                            </BarChart>
                          </ResponsiveContainer>
                        ...
                        {MOCK_REPORTS.map((report) => (
                          <Card key={report.id} className="rounded-3xl border-none shadow-soft bg-card hover:shadow-premium transition-all group cursor-pointer">
                            <CardContent className="p-6 space-y-4">
                              <div className="size-10 rounded-2xl bg-muted/50 flex items-center justify-center group-hover:bg-brand-blue/10 transition-colors">
                                <FileText className="size-5 text-muted-foreground group-hover:text-brand-blue" />
                              </div>
                              <div className="space-y-1">
                                <p className="text-sm font-bold leading-tight line-clamp-2">{report.title}</p>
                              ...
                              <div className="flex items-center justify-between pt-2 border-t border-border/5">
                                <span className="text-[10px] font-bold text-muted-foreground">{report.size}</span>
                ...
                <motion.div variants={itemVariants} className="pb-12 pt-8 text-center">
                  <div className="inline-flex items-center gap-3 px-6 py-3 rounded-2xl bg-brand-blue/5 border border-brand-blue/10">
                    <p className="text-xs font-semibold text-brand-blue">All models are re-validated daily against historical settlement data.</p>
                    <ArrowRight className="size-3 text-brand-blue" />
                  </div>
                </motion.div>
              </motion.div>
            )}
