          ...
          <BarChart data={data} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                    ...
                    <div className="glass-premium p-4 rounded-2xl border-none shadow-premium min-w-[140px]">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className={cn(
                          ...
                          isPositive ? "text-gain-600" : isNegative ? "text-loss-600" : "text-muted-foreground"
                        )}>
                          {formatPct(val)}
                        </span>
                      </div>
            ...
            >
              {data.map((entry, index) => {
                const isPositive = entry.value > 0.005;
                const isNegative = entry.value < -0.005;
                return (
                  <Cell
                    key={`cell-${index}`}
                    fill={isPositive ? '#14B8A6' : isNegative ? '#FF3B30' : '#94A3B8'}
                    fillOpacity={0.8}
                  />
                );
              })}
